  ...
  function embedCodeDialog ($rootScope, $mdDialog, $window, youtubePlayerAPI, codeGenerator, layoutManager) {
    ...
    self.open = function (event, type) {
      ...
      $mdDialog.show(codeDialog).finally(function () {
        codeDialog = angular.isUndefined
      })
    }
    ...
    function onOpen () {
      ...
      code.create().then(
            ...
            function () { $rootScope.$broadcast('embedCodeDialog:database-error') }
        )
    };
  };
...
// -------------------------------------------- CONTROLLER ------------------------------------------------
  ...
  */
  function CodeDialogController ($scope, $mdDialog, $window, $timeout, codeGenerator, youtubePlayerAPI, layoutManager) {
