(function () {
  ...
  function youtubePlayerAPI ($rootScope, $timeout, $interval) {
    ...
    self.mobile = (!!navigator.userAgent.match(/(iPad|iPhone|iPod|Android)/g))
    self.phone = (!!navigator.userAgent.match(/(iPod|iPhone|Android)/g))
    ...
    function timeoutLength () {
      ...
      if (self.currentRate === 1.5) return Math.floor(ms * 0.66)
      return ms
    }
    /**
    ...
    self.togglePlay = function () {
      ...
      if (self.state === PLAYING) {
        ...
        if ((self.time() + 0.25) > self.endpoint.val) {
          self.seek(self.startpoint.val)
          self.play()
        } else {
          self.play()
    ...
    self.onPlayerStateChange = function (event) {
      if (event.data === PLAYING) {
        ...
        } else if ((self.time() + 0.25) > self.endpoint.val) {
          self.pause()
          self.seek(self.startpoint.val)
          self.play()
      ...
      if (event.data === PLAYING) {
        ...
        if (self.initializing) {
          ...
          if (!self.android) {
            ...
            $timeout(function () {
              self.initializing = false
              self.setStartpoint(mobileStartTime)
              self.seek(mobileStartTime)
              $rootScope.$broadcast(updateEvent.name)
              $rootScope.$broadcast(readyEvent.name)
            }, 1000)
